variable "name" {
  description = "The name of the Route53 Resolver Inbound Endpoint."
  type        = string
}

variable "security_groups" {
  description = "A list of security groups for Route53 Resolver Inbound Endpoint."
  type        = list(string)
  default     = []
}

variable "ip_allocations" {
  description = "A list of IP allocations which include `subnet_id` and `ip`. `ip` is a private IP from `subnet`."
  type = list(object({
    subnet_id = string
    ip        = string
  }))
...
variable "allowed_ingress_cidrs" {
  description = "A list of CIDR for Route53 Resolver Inbound Endpoint ingress access."
  type        = list(string)
  default     = ["0.0.0.0/0"]
}
